...
static load_cb _cb = NULL;
static close_cb _ccb = NULL;
...
//加载文件
static int luaL_loadfilex_(lua_State *L, const char *filename, const char *mode) {
    LoadF lf;
    int status, readstatus;
    int c;
    int fnameindex = lua_gettop(L) + 1;  /* index of filename on the stack */
    //先从加载 器加载
    if (filename != NULL && _cb) {
        LoadS ls = { 0 };
        ls.size = _cb(filename, &ls.s);
        if (ls.size) {
            lua_pushfstring(L, "@%s", filename);
            int ret= lua_load(L, getS, &ls, lua_tostring(L, -1), NULL);
            if (_ccb)
                _ccb(ls.s);
            return ret;
        }
...
//初始化加载器
LUALIB_API int luaL_loadinit(load_cb cb, close_cb ccb) {
    _cb = cb;
    _ccb = ccb;
}
/* }====================================================== */
...
static const void *load(const char *key) {
    if (CC.L == NULL)
        return NULL;
    SPIN_LOCK(&CC);
    lua_State *L = CC.L;
...
static const void *save(const char *key, const void * proto) {
    lua_State *L;
