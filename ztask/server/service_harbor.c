...
hash_erase(struct hashmap * hash, char name[GLOBALNAME_LENGTH]) {
    uint32_t *ptr = name;
    uint32_t h = ptr[0] ^ ptr[1] ^ ptr[2] ^ ptr[3];
    struct keyvalue ** nptr = &hash->node[h % HASH_SIZE];
    while (*nptr) {
        struct keyvalue * node = *nptr;
        if (node->hash == h && strncmp(node->key, name, GLOBALNAME_LENGTH) == 0) {
            _release_queue(node->queue);
            ((struct keyvalue *)*nptr)->next = node->next;
            ztask_free(node);
            return;
        }
        *nptr = &(node->next);
...
harbor_command(struct harbor * h, const char * msg, size_t sz, int session, uint32_t source) {
    ...
    case 'A': {
        ...
        struct slave * slave = &h->s[id];
        if (slave->fd > 0) {
            ztask_error(h->ctx, "节点 %d 已连接", id);
            return;
        }
        slave->fd = fd;

        ztask_socket_start(h->ctx, 0, fd);
        handshake(h, id);
        if (msg[0] == 'S') {
...
mainloop(struct ztask_context * context, void * ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
    ...
    default:
        ztask_error(context, "recv invalid message from %x,  type = %d", source, type);
        if (session != 0 && type != PTYPE_ERROR) {
            ztask_send(context, 0, source, PTYPE_ERROR, session, NULL, 0);
        }
        return 0;
    }
}
