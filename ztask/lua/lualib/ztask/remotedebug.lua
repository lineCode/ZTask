...
local injectrun = require "ztask.injectcode"
local table = table
local debug = debug
...
local ctx_ztask = debug.getinfo(ztask.start,"S").short_src	-- skip when enter this source file
...
function linehook(mode, line)
	...
	if mode ~= "line" then
		ctx.needupdate = true
		if mode ~= "return" then
			if ctx.next_mode or debug.getinfo(2,"S").short_src == ctx_ztask then
				ctx.level = 1
		...
		if ctx.needupdate then
			...
			if ctx.filename == ctx_term then
				ctx_active[co] = nil
				sethook()
				change_prompt(string.format(":%08x>", ztask.self()))
				return
			end
		end
...
local function watch_proto(protoname, cond)
	local proto = assert(replace_upvalue(ztask.register_protocol, "proto"), "Can't find proto table")
...
function dbgcmd.s(co)
	local ctx = ctx_active[co]
	ctx.next_mode = false
	ztask_suspend(co, coroutine.resume(co))
end
	...
	if func then
		local function idle()
			if raw_dispatcher then
			    ztask.timeout(10,idle)	-- idle every 0.1s
			end
		end
		ztask.timeout(0, idle)
	end
	return func
end
...
function M.start(import, fd, handle)
	...
	ztask_suspend = import.suspend
	assert(raw_dispatcher == nil, "Already in debug mode")
	ztask.error "Enter debug mode"
	local channel = debugchannel.connect(handle)
	raw_dispatcher = hook_dispatch(dispatcher, ztask.response(), fd, channel)
end
