	...
	while self.__sock do
		...
		if ok and session then
			...
			if co then
				...
				self.__thread[session] = nil
				ztask.error("socket: unknown session :", session)
			end
		else
...
local function pop_response(self)
	while true do
		...
		ztask.wait(self.__wait_response)
	end
end
...
local function push_response(self, response, co)
	if self.__response then
		...
		if self.__wait_response then
			ztask.wakeup(self.__wait_response)
...
local function connect_backup(self)
	if self.__backup then
		for _, addr in ipairs(self.__backup) do
			...
			ztask.error("socket: connect to backup host", host, port)
...
local function connect_once(self)
	...
	self.__dispatch_thread = ztask.fork(dispatch_function(self), self)
	...
	if self.__auth then
		...
		if not ok then
			...
			if message ~= socket_error then
				...
				ztask.error("socket: auth failed", message)
			end
		end
		self.__authcoroutine = false
...
local function try_connect(self , once)
	...
	while not self.__closed do
		...
		if ok then
			if not once then
				ztask.error("socket: connect to", self.__host, self.__port)
			end
			return
		elseif once then
			return err
		else
			ztask.error("socket: connect", err)
		end
		if t > 1000 then
			ztask.error("socket: try to reconnect", self.__host, self.__port)
			ztask.sleep(t)
			t = 0
		else
			ztask.sleep(t)
...
local function check_connection(self)
	if self.__sock then
		if socket.disconnected(self.__sock[1]) then
			-- closed by peer
			ztask.error("socket: disconnect detected ", self.__host, self.__port)
			close_channel_socket(self)
			return
		end
...
local function block_connect(self, once)
	...
	if r == nil then
		ztask.error(string.format("Connect to %s:%d failed (%s)", self.__host, self.__port, err))
		error(socket_error)
	else
		return r
	end
